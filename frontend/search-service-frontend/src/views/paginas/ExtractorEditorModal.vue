<template>
    <div class="modal fade" :id="`extractor-editor-${type}`" data-bs-backdrop="static" data-bs-keyboard="false"
        tabindex="-1" aria-labelledby="extractor-editor-label" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="extractor-editor-label">
                        {{ type }} Extractor
                        <small class="text-body-secondary">{{ titulo }}</small>
                    </h1>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label>
                            Subir archivo javascript con extractor
                    ...
                    <textarea name="content" cols="30" rows="10" class="form-control" @change="() => { changed = true; }" v-model="newContent"></textarea>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" :disabled="!changed" @click="editarPagina" data-bs-dismiss="modal">Guardar</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    created(){
        this.newPagina.id = this.paginaId
